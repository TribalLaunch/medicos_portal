...
export default function ProductDetail() {
  // ✅ Hooks must always run, every render, before any early returns
  const { sku } = useParams();
  ...
  const {
    ...
    queryFn: () => getProductBySku(sku!), // safe because enabled guards it
    enabled: !!sku,
  });

  // Derived values must be safe when data is undefined
  ...
  const safePrimaryIndex = useMemo(() => {
    ...
    return Math.max(0, Math.min(p, data.images.length - 1));
  }, [data]);

  const safeIndex = useMemo(() => {
    if (!data?.images?.length) return 0;
    if (activeIndex >= 0 && activeIndex < data.images.length) return activeIndex;
    return safePrimaryIndex;
  }, [activeIndex, data, safePrimaryIndex]);

  const msrp = useMemo(() => (data ? Number(data.priceMSRP) : 0), [data]);

  // If backend later includes contract price, we’ll prefer it when logged in
  const contractPrice = (data as any)?.contractPrice;
  const cartPrice = useMemo(() => {
    if (!data) return 0;
    if (user && contractPrice !== undefined && contractPrice !== null) {
      return Number(contractPrice);
    }
    return Number(data.priceMSRP);
  }, [data, user, contractPrice]);

  const mainSrc = images[safeIndex] || DEFAULT_PRODUCT_IMG;

  // ✅ After hooks, it's safe to return early
  if (!sku) return <div className="card">Missing SKU.</div>;
  if (isLoading) return <div className="card">Loading…</div>;
  if (error) return <div className="card text-red-700">{(error as Error).message}</div>;
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <div className="space-y-4">
        ...
        <div className="text-sm text-gray-500">Manufacturer: {data.manufacturer}</div>
        <h1 className="text-2xl font-semibold">{data.name}</h1>

        <div className="space-y-1">
          <div className="text-3xl font-bold">${msrp.toFixed(2)}</div>
          {user && contractPrice !== undefined && contractPrice !== null && (
            <div className="text-sm">
              <span className="badge mr-2">Contract</span>
              <span className="font-semibold">${Number(contractPrice).toFixed(2)}</span>
            </div>
          )}
        </div>

        {data.requiresPrescription && (
          <div className="text-sm text-red-700">⚠️ This item requires a prescription.</div>
        )}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  addItem({
                    ...
                    imageUrl: (resolveProductImage(data, safePrimaryIndex) || DEFAULT_PRODUCT_IMG),
